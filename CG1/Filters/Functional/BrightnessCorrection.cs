...
namespace CG1
{
    class BrightnessCorrection : Filter
    {
        public string Name { get { return "Brightness correction"; } }
        private double brighnessValue;

        //brightnessValue - from -1 to 1
        public BrightnessCorrection(double brightness)
        {
  
            brighnessValue = brightness;
        }
            ...
            for (int y = 0; y < bmp.Height; y++)
            {
                for (int x = 0; x < bmp.Width; x++)
                {
                    ...
                    for (int i = 0; i < cChannels; i++)
                    {
                        ...
                        if (brighnessValue >= 0)
                            result[current + i] = (byte)((255 - channel) * brighnessValue + channel);
                        else
                        {
                            brighnessValue = 1 + brighnessValue;
                            result[current + i] = (byte)(brighnessValue * channel);
                        }
